...
"sap/m/MessageBox",
"sap/m/MessageToast"
], function(Controller,MessageToast,Filter,FilterOperator,MessageBox,MessageToast) {
    'use strict';
    return Controller.extend("sap.ui.demo.controller.View2",{
        ...
        isSupplierPopup:function(id){
            if(id.indexOf("suppliersPopup")!= -1){
                return true;
            }
            else{
                return false;
        ...
        onSearch:function(oEvent){
            ...
            if(this.isSupplierPopup(sId)){
                let oFilter = new Filter("CompanyName",FilterOperator.Contains,oEvent.getParameter("value"));
                this.oPopupSuppliers.getBinding("items").filter(oFilter);
            }
            else{
                let oFilter = new Filter("city",FilterOperator.Contains,oEvent.getParameter("value"));
                this.oPopupCities.getBinding("items").filter(oFilter);
                }
        },
        oncancel:function(oEvent){
            this.getView().byId("idSuppliers").getBinding("items").filter([]);
        }
    });
});